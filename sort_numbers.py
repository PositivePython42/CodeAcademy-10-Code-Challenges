"""
Sort a list Create a function in Python that accepts two parameters.
The first will be a list of numbers. The second parameter will be a string that can be one of the following values: asc, desc, and none.
If the second parameter is “asc,” then the function should return a list with the numbers in ascending order. If it’s “desc,” then the list should be in descending order, and if it’s “none,” it should return the original list unaltered.
"""

number_list = []

def sort(number_list, order):
    if sort_order == 'asc':
        number_list.sort()
        print(number_list)
    elif sort_order == 'desc':
        number_list.sort(reverse=True)
        print(number_list)
    else:
        print(number_list)
        
for i in range(0, 10):
    number = input("Please enter a number for you list : ")
    number_list.append(number)

sort_order = input("Do you want to sort ascending (asc), descending (desc) or no sort (none) : ")

sort(number_list, sort_order)